...
(function (global) {
  ...
  var MARGIN = 10;          // small outer breathing room; axis labels now
                            // render INSIDE the plot rect (see buildFrame)
                            // rather than consuming a large outer margin,
                            // so the plotted square fills nearly the full
                            // card width.
  var CENTER = SIZE / 2;
   ...
   * clipPath (id clipId) other content can be clipped to. Longitude tick
   * labels run along the inside of the bottom edge; latitude tick labels
   * run along the inside of the left edge -- both drawn just inside the
   * border (not in an outer margin) so the plotted square itself uses the
   * full available width, with a small translucent backdrop behind each
   * label so it stays legible over gridlines/plotted content.
   */
  function buildFrame(originLat, originLon, scale, clipId) {
    ...
    GRID_FRACTIONS.forEach(function (f) {
      ...
      if (isCenter || isEdge) {
        ...
        // Longitude ticks: inside the bottom edge. Nudge the two extreme
        // ticks inward horizontally so their label chips don't overhang
        // past the plot's left/right border.
        var lonAnchor = 'middle';
        var lonPx = px;
        if (f === -1) { lonAnchor = 'start'; lonPx = px + 3; }
        if (f === 1) { lonAnchor = 'end'; lonPx = px - 3; }
        var lonY = PLOT_MAX - 6;
        var lonText = formatLon(lonHere);
        var lonChipW = 6 + lonText.length * 5.6;
        var lonChipX = lonAnchor === 'start' ? lonPx : (lonAnchor === 'end' ? lonPx - lonChipW : lonPx - lonChipW / 2);
        lonLabels += '<rect x="' + lonChipX + '" y="' + (lonY - 10) + '" width="' + lonChipW + '" height="13" rx="2" fill="var(--bg)" fill-opacity="0.72"/>';
        lonLabels += '<text x="' + lonPx + '" y="' + lonY + '" text-anchor="' + lonAnchor + '" class="chart-axis-label">' + lonText + '</text>';

        // Latitude ticks: inside the left edge. Nudge the two extreme ticks
        // vertically inward so their chips don't overhang top/bottom.
        var latPy = py;
        if (f === 1) latPy = py + 9;   // north tick -> nudge down, away from top border
        if (f === -1) latPy = py - 4;  // south tick -> nudge up, away from bottom border (and clear of the longitude row)
        var latX = PLOT_MIN + 4;
        var latText = formatLat(latHere);
        var latChipW = 6 + latText.length * 5.6;
        latLabels += '<rect x="' + (latX - 2) + '" y="' + (latPy - 9) + '" width="' + latChipW + '" height="13" rx="2" fill="var(--bg)" fill-opacity="0.72"/>';
        latLabels += '<text x="' + latX + '" y="' + (latPy + 3.5) + '" text-anchor="start" class="chart-axis-label">' + latText + '</text>';
      }
    });

    return svg + lonLabels + latLabels;
  }
